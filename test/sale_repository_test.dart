...
void main() {
  ...
  test('processCheckout (Cash) records sale and deducts stock', () async {
    await itemRepo.addItem(
      ...
      sellingPrice: const Money(100),
      stockQuantity: 10,
    );
    final items = await itemRepo.watchAllActiveItems().first;
    final item = items.first;

    final cart = CartState(items: [CartLineItem(item: item, quantity: 3)]);
  ...
  test(
    'processCheckout (Credit) updates customer balance and records transaction',
    () async {
      await itemRepo.addItem(
        name: 'Banana',
        type: ItemType.product,
      ...
      final txs = await db.creditTransactionDao
          .watchTransactionsForCustomer(cust.id)
          .first;
      expect(txs.length, 1);
