...
void main() {
  ...
  test('P&L calculation and date slicing', () async {
    // 1. Setup Data
    await custRepo.addCustomer(name: 'Test Customer', creditLimit: const Money(10000));
    ...
    // Repayment
    await db.into(db.creditTransactions).insert(CreditTransactionsCompanion.insert(
    ...
    await (db.update(db.customers)..where((c) => c.id.equals(customer.id))).write(
      CustomersCompanion(
         currentBalanceCents: drift.Value(customer.currentBalance.cents + 1000 - 500),
    ...
    // Expenses = 300
    // Net = 500
    final accrualPl = await analyticsRepo.getProfitAndLoss(now - 10000, now + 10000, isCashBasis: false);
    ...
    // Expenses = 300
    // Net = 0
    final cashPl = await analyticsRepo.getProfitAndLoss(now - 10000, now + 10000, isCashBasis: true);
