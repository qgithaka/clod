...
class _DocumentFormViewState extends ConsumerState<DocumentFormView> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text('Create ${widget.documentType.toUpperCase()}')),
  ...
  Future<void> _showAddItemDialog(BuildContext context, List<dynamic> activeItems) async {
    int? selectedItemId;
    int qty = 1;
               ...
               content: Column(
                 ...
                 children: [
                   DropdownButtonFormField<int>(
                     ...
                     items: activeItems.map((i) => DropdownMenuItem(value: i.id as int, child: Text(i.name as String))).toList(),
                 ...
                 FilledButton(
                   onPressed: () {
                     if (selectedItemId != null && qty > 0) {
                       final item = activeItems.firstWhere((i) => i.id == selectedItemId);
                       this.setState(() {
                         _items.add(DocumentLineItem(
