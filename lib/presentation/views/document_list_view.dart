...
final documentListProvider = StreamProvider.family<List<DocumentEntity>, String>((ref, type) {
...
class _DocumentListViewState extends ConsumerState<DocumentListView> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: docsAsync.when(
        data: (docs) {
          if (docs.isEmpty) return Center(child: Text('No ${_selectedType}s found.'));
          return ListView.builder(
            itemCount: docs.length,
            itemBuilder: (context, index) {
              final d = docs[index];
              return ListTile(
                leading: CircleAvatar(child: Icon(_selectedType == 'quote' ? Icons.request_quote : Icons.receipt)),
                ...
                trailing: Text(Money(d.totalAmountCents).format(), style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                isThreeLine: true,
                onTap: () {
                          ...
                          children: [
                            ListTile(
                              ...
                              onTap: () {
                                ...
                                ref.read(documentPdfServiceProvider).generateAndShareDocument(d);
                              },
                            ),
                            if (d.type == 'quote' && d.status == 'draft')
                              ListTile(
                                ...
                                onTap: () {
                                  Navigator.pop(context);
                                  ref.read(documentRepositoryProvider).convertQuoteToInvoice(d.id);
                                },
                              ),
                            if (d.type == 'quote')
                              ListTile(
                                ...
                                onTap: () async {
                                  ...
                                  try {
                                    ...
                                    final customer = await customerRepo.watchCustomer(d.customerId).first;
                                  ...
                                  if (mounted) {
                                    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Added to Cart! (Partial logic for now)')));
                                    context.go('/pos');
                                  }
                                },
                              ),
