...
class ItemRepository {
  ...
  Stream<List<ItemEntity>> watchAllActiveItems() {
    return _dao.watchAllActiveItems().map((list) => list.map((i) => ItemEntity.fromData(i)).toList());
  }

  Stream<ItemEntity> watchItem(int id) {
    return _dao.watchAllActiveItems().map((list) => list.firstWhere((i) => i.id == id)).map((i) => ItemEntity.fromData(i));
  }
  ...
  Future<void> addItem({
    ...
    await _dao.insertItem(ItemsCompanion.insert(
      name: name,
  ...
  Future<void> updateItem(
    ...
    final now = DateTime.now().toUtc().millisecondsSinceEpoch;
    await _dao.updateItem(ItemsCompanion(
      id: Value(id),
      name: Value(name),
