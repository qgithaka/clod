...
class PurchaseRepository {
  ...
  Future<int> createDraftPO(List<PurchaseOrderLine> lines) async {
    ...
    return await _db.transaction(() async {
      final poId = await _db.purchaseDao.insertPurchase(PurchasesCompanion.insert(
        status: 'draft',
  ...
  Future<void> markAsReceived(int poId) async {
    ...
    await _db.transaction(() async {
      ...
      await (_db.update(_db.purchases)..where((tbl) => tbl.id.equals(poId)))
          .write(PurchasesCompanion(status: const Value('received'), receivedAt: Value(now)));

      // 2. Process each item: update stock, insert movement, update average cost
      final items = await (_db.select(_db.purchaseItems)..where((tbl) => tbl.purchaseId.equals(poId))).get();
      ...
      for (final poItem in items) {
        ...
        final averageCostCents = totalQty > 0 ? (totalOldValue + totalNewValue) ~/ totalQty : 0;
  ...
  Stream<List<Purchase>> watchPurchases(String status) {
    return (_db.select(_db.purchases)..where((t) => t.status.equals(status))).watch();
  }
}
