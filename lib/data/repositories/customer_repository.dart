...
class CustomerRepository {
  ...
  Stream<List<CustomerEntity>> watchAllCustomers() {
    return _dao.watchAllCustomers().map(
      (list) => list.map((c) => CustomerEntity.fromData(c)).toList(),
    );
  }
  ...
  Stream<CustomerEntity> watchCustomer(int id) {
    ...
    return _dao
        .watchAllCustomers()
        .map((list) => list.firstWhere((c) => c.id == id))
        .map((c) => CustomerEntity.fromData(c));
  }
  ...
  Stream<List<CreditTransactionEntity>> watchTransactions(int customerId) {
    return _creditDao
        ...
        .map(
          (list) =>
              list.map((t) => CreditTransactionEntity.fromData(t)).toList(),
        );
  }
  ...
  Future<void> recordRepayment(
    ...
    await db.transaction(() async {
      ...
      await _creditDao.insertTransaction(
      ...
      await _dao.updateCustomer(
        customer
            .copyWith(currentBalanceCents: newBalance, updatedAt: now)
            .toCompanion(true),
      );
    });
  }
...
final customerRepositoryProvider = Provider<CustomerRepository>((ref) {
  return CustomerRepository(
    ...
    ref.watch(creditTransactionDaoProvider),
  );
});
...
final creditTransactionsProvider =
    StreamProvider.family<List<CreditTransactionEntity>, int>((
      ref,
      customerId,
    ) {
      return ref
