...
class BusinessProfileDao extends DatabaseAccessor<AppDatabase>
  ...
  Stream<BusinessProfileData?> watchProfile() =>
      (select(businessProfile)..limit(1)).watchSingleOrNull();
  Future<BusinessProfileData?> getProfile() =>
      (select(businessProfile)..limit(1)).getSingleOrNull();
  Future<int> insertOrUpdate(BusinessProfileCompanion profile) =>
      into(businessProfile).insertOnConflictUpdate(profile);
}
...
class ItemDao extends DatabaseAccessor<AppDatabase> with _$ItemDaoMixin {
  ...
  Stream<List<Item>> watchAllActiveItems() =>
      (select(items)..where((i) => i.isActive.equals(true))).watch();
...
class CreditTransactionDao extends DatabaseAccessor<AppDatabase>
  ...
  Future<int> insertTransaction(CreditTransactionsCompanion txn) =>
      into(creditTransactions).insert(txn);
}
...
class StockMovementDao extends DatabaseAccessor<AppDatabase>
  ...
  Stream<List<StockMovement>> watchMovementsForItem(int itemId) {
    return (select(
      stockMovements,
    )..where((m) => m.itemId.equals(itemId))).watch();
  }
